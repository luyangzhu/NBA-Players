...
class ShapeData(object):
    def __init__(self, train_file, val_file, test_file, reference_mesh_file, normalization = True, meshpackage = 'mpi-mesh', load_flag = True, mean_subtraction_only = False):
        ...
        if self.meshpackage == 'trimesh':
            # some problems in trimesh obj loader, need to bypass it
            v,vt,v_idx,vt_idx = readObjV2(reference_mesh_file)
            v_idx -= 1
            self.reference_mesh = trimesh.base.Trimesh(vertices=v, faces=v_idx, process = False)
    ...
    def load(self):
        ...
        self.vertices_val = np.load(self.val_file)
