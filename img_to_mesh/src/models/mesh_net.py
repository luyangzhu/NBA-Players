...
class SpiralTLNetV2(nn.Module):
    def __init__(self, filters_enc, filters_dec, latent_size, sizes, spiral_sizes, nParts, spirals, D, U, device, activation = 'elu'):
        super(SpiralTLNetV2,self).__init__()
        ...
        # j3d fc
        self.bilinear1 = Bilinear(self.nParts*3)
        self.fc1 = nn.Linear(self.nParts*3, latent_size)
        ...
        for i in range(len(spiral_sizes)-1):
            if filters_enc[1][i]:
                self.conv_rest_pose.append(SpiralConv(input_size, spiral_sizes[i], filters_enc[1][i],
                                            activation=self.activation, device=device).to(device))
                input_size = filters_enc[1][i]

            self.conv_rest_pose.append(SpiralConv(input_size, spiral_sizes[i], filters_enc[0][i+1],
                                        activation=self.activation, device=device).to(device))
            input_size = filters_enc[0][i+1]

        self.conv_rest_pose = nn.ModuleList(self.conv_rest_pose)
        self.fc_rest_pose = nn.Linear((sizes[-1]+1)*input_size, latent_size)
        self.fc_concat = nn.Linear(latent_size*2, latent_size)
    ...
    def encode_rest_pose(self,x):
        ...
        j = 0
        for i in range(len(self.spiral_sizes)-1):
            x = self.conv_rest_pose[j](x,S[i].repeat(bsize,1,1))
            j+=1
            if self.filters_enc[1][i]:
                x = self.conv_rest_pose[j](x,S[i].repeat(bsize,1,1))
                j+=1
    ...
    def forward(self, verts, j3d, rest_verts):
        bsize = verts.size(0)
        z_v = self.encode(verts)
        pred_verts_v = self.decode(z_v)
        j3d = j3d.view(bsize,-1)
        z_j = self.bilinear1(j3d)
        z_j = self.fc1(z_j)
        z_rest = self.encode_rest_pose(rest_verts)
        z_concat = torch.cat((z_j, z_rest), 1)
        z_concat = self.fc_concat(z_concat)
        pred_verts_j = self.decode(z_concat)
        return z_v, pred_verts_v, z_concat, pred_verts_j

    def test(self, j3d, rest_verts):
        bsize = j3d.size(0)
        j3d = j3d.view(bsize,-1)
        z_j = self.bilinear1(j3d)
        z_j = self.fc1(z_j)
